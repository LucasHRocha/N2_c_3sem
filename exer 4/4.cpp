...
//---------------------------------------------------------------        
 int somaElementos(Arv *a){
	...
	return x;// retorna a variavel pra função principal
 }
//----------------------------------------------------------------
 void elementosSemfilhos(Arv *a){// tipo void 
 	...
 	if(a!=NULL){
		if (a->esq==NULL && a->dir==NULL){ // se o da elemento da esqueda e da direita for vazio 
				printf("%d\t",a->matricula); // então o elemento não tem filhos
		}
...
int conta(Arv *T, float nota){
	int cont=0;
	if(T!=NULL){
		if(T->nota > nota) // compara de a nota dada é menor que a nota da árvore
			cont= cont+1;
		cont= cont+conta(T->esq,nota); // vai pra esquerda e verifica
		cont= cont+conta(T->dir,nota); // vai pra direita e verifica
	}
...
void menu(){
	...
	printf("*******************\n");
	printf(" 1 - Insere\n");
	printf(" 2 - Consulta quais notas maior que as selecionadas\n");
	printf(" 3 - Mostra\n");
	printf(" 4 - Busca\n");
	...
	do{
		...
		switch(op){
			...
			case 2: //retira
			     ...
			     scanf("%f",& nota);
			     //mostra(a,nota);
			     printf("A quantidade de notas mairo que %.2f e %d: ",nota,conta(a,nota));
			     break;
			/*case 3: //mostra
